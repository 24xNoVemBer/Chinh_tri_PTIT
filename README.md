...
| `RAG_RETRIEVAL_PROFILE`                   | `legacy-v1`                           | `legacy-v1` để rollback; `top5-v2` dùng tối đa 5 chunk và ngân sách token        |
...
| `MODEL_ID`                                | `gpt-4o-mini`                         | Groq chat-only dùng `openai/gpt-oss-20b`                                         |
| `EMBEDDING_MODEL_ID`                      | `text-embedding-3-large`              | Groq đặt `none`; retrieval tiếp tục dùng BM25                                    |
